...
- **Query Transformation**: Utilizes a dedicated lightweight LLM (configurable via `QUERY_TRANSFORM_MODEL` / `AZURE_QUERY_TRANSFORM_DEPLOYMENT`, defaults to nano-tier) for intent expansion and spell-checking to solve multi-lingual vector space misalignment. The transformer runs on a separate model from the Answer LLM — keeping inference costs low while the answer pipeline runs a more capable model. Generates `german_query`, `english_query`, and `query_variants` simultaneously, searching across all of them for maximum recall.
...
- **LLM Factory Pattern (Multi-Provider)**: Supports **OpenAI** and **Azure OpenAI** interchangeably via a single `USE_AZURE_OPENAI` toggle. The pipeline uses **dual-model routing** — the Answer LLM (`OPENAI_MODEL` / `AZURE_LLM_DEPLOYMENT`) and the Query Transformer (`QUERY_TRANSFORM_MODEL` / `AZURE_QUERY_TRANSFORM_DEPLOYMENT`) are independently configurable, allowing a nano-tier model for cheap classification and a stronger model for answer generation. If the primary provider is offline, the system can seamlessly fall back to a local **Ollama** model (ideal for local resilience testing).
