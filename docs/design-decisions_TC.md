...
### 決策 4 — REQ:2-7 語言分離：將德文積分與英文 C1 加分區隔

**背景：** Chancenkarte 積分系統允許同時累積德文語言積分（`REQ:2-1`，最高 4 分，透過 `A2|1`/`B1|2`/`B2|3`/`C1|4`）與英文 C1 加分（`REQ:2-7`，+1 分，透過 `EN_C1|1`）。兩者是獨立的評分維度，絕不能共用同一個標籤 ID。

**問題：** 模型持續將英文 C1 加分錯誤地輸出為 `REQ:2-1`，而非 `REQ:2-7`，導致 UI 介面和積分計算器無法區分德文與英文語言的貢獻。

識別出兩個相互疊加的根本原因：

1. **ID 參照表缺漏：** `REQ:2-7` 不在 `prompt_builder.py` 的權威性 REQ ID 參照表中，模型對英文 C1 標籤沒有規範性的錨點，因而預設使用最相近的已知語言 ID（`REQ:2-1`）。
2. **模型強先驗 (Strong Prior)：** 即使在修正 ID 參照表之後，情境 1（純英文 C1，完全沒有德文）依然不穩定。模型訓練時形成的先驗——將 C1 程度與單一語言標籤關聯——在特定對話模式下能覆蓋提示詞層級的指示。

**決策 — 五層實作：**

- **Layer 1（提示詞）：** 在 Chancenkarte ID 參照表中加入 `REQ:2-7`。新增 `MANDATORY SPLIT` 區塊並附上純英文情境與疊加情境的 WRONG/CORRECT 範例。將 Path 1 豁免清單延伸，納入 `REQ:2-7`。
- **Layer 4（確定性 Filter）：** 在 `tag_filter.py` 中新增 `apply_english_c1_split_filter`。任何值在 `{"C1|1", "EN_C1|1"}` 範圍內的 `REQ:2-1` 輸出，將靜默地重新路由為 `REQ:2-7:EN_C1|1`。同時將 `apply_path1_filter` 的壓制集合從 `{"1-2"}` 擴展至 `{"1-2", "2-1", "2-7"}`。
- **Layer 2（前端 Store）：** 在 `chatStore.ts` 中加入 `REQ:2-7` 欄位。將 REQ:2-1 的標籤從 `Language` 更名為 `Language (German)`。
- **Layer 3（UI 邏輯）：** 更新 `InsightsPanel.tsx`，讓正向/反向推論皆將 `2-1` 和 `2-7` 視為語言類 REQ。從積分映射表中移除 `en_c1`（現在直接透過 REQ:2-7 處理）。
- **Layer 5（測試）：** 新增 27 個單元測試，覆蓋兩個 Filter 及新的提示詞渲染行為。

**為何確定性 Filter 不可或缺：** 情境 2（德文 + 英文疊加）在提示詞修正後即穩定運作。情境 1（純英文，無德文）則未能做到——單靠提示詞指示，無法在訓練分佈中代表性不足的情境下覆蓋足夠強烈的模型先驗。該 Filter 能確保正確性，不受模型輸出波動影響。

**安全不變量：**
- 德文數值 `A2|1`、`B1|2`、`B2|3`、`C1|4` 與 `_ENGLISH_C1_VALUES = {"C1|1", "EN_C1|1"}` 無交集，因此不會有德文積分被靜默重新路由。
- Path 1 正確壓制所有三個語言相關 ID（`{"1-2", "2-1", "2-7"}`），因為在直接認可（Direct Recognition）路徑下，語言門檻完全豁免。
