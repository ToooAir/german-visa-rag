...
1. **多語言 Dense Embedding 模型**：底層採用 `text-embedding-3-small`，該模型在同一向量空間內具備基礎的 Cross-lingual Alignment 能力。**未選用 `text-embedding-3-large` 或 `Multilingual-E5` 的考量在於：經過實測，在簽證法規這類有限的 Domain 領域中，`small` 模型的語義解析度已相當充足，且在 API 呼叫的延遲（Latency）與成本（Cost）上具備明顯優勢，更符合目前的專案規模。**
...
- **No-Context Fallback 與溯源強制 (Backend)**：
  - 當 RAG 檢索不到相關資訊時，強制進入 Fallback 流程，指示 LLM 向使用者表明「知識庫無此資訊」，禁止瞎掰。
  - 要求每句事實陳述皆須附上來源文件連結（Citation），並在檢索權重上給予 `[OFFICIAL]` 官方文件分數加權。
- **建立用戶信任的透明度設計 (Frontend/UX)**：
  - 僅有 Backend 壓制幻覺仍不夠，必須從 UX 視角建立信任。前端實作了 **Knowledge Base Browser**，讓生成的 Citation 不僅是文字連結，用戶可以直接點閱並在側欄查閱「原始法規出處原文」。把「驗證的權力」交還給用戶，大幅降低對 LLM 產出的疑慮。
