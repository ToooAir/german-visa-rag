...
### 模型基準測試 (Model Benchmark) — gpt-4o-mini vs gpt-4.1-mini vs gpt-5-mini (2026-04-18)

為了替下一次線上部署選擇合適的回答 LLM，我們針對三款由 Azure 託管的模型進行了基準測試。測試使用串流模式對兩個具備代表性的測試案例進行，測量指標包含 TTFT（首字延遲）、總體延遲、回應長度以及 State Tag F1 分數。gpt-5-mini 分別在完全推理（full-reasoning）以及 `reasoning_effort: "none"` 兩種配置下進行測試。

**針對推理模型系列（gpt-5 / o1 / o3）於 `src/llm/openai_client.py` 實作的相容性修正：**

| 議題 | 根本原因 | 修復方式 |
| :--- | :--- | :--- |
| `400 Unsupported parameter: max_tokens` | 推理模型需要使用 `max_completion_tokens` | 透過 `model.startswith(("o1","o3","gpt-5"))` 自動偵測 |
| `400 Unsupported value: temperature 0.1` | 僅支援預設溫度 (1) | 針對限制模型系列省略 `temperature` 與 `top_p` 參數 |
| API 成功但回應為空 | 1024 標記（tokens）的預算被內部推理消耗殆盡，導致輸出內容空間為 0 | 強制執行最小 `max_completion_tokens=4096` |

**基準測試結果（2 個測試案例，各執行單次）：**

| 模型 | 平均 TTFT | 平均 總時長 | 平均 字數 | Macro F1 |
| :--- | :---: | :---: | :---: | :---: |
...
| 模型 | CK T0 F1 | Path1 T0 F1 | 備註 |
| :--- | :---: | :---: | :--- |
| gpt-4o-mini | 0.889 | 0.857 | 穩定；一個 FN（遺漏 REQ:2-1 分數標籤） |
| gpt-4.1-mini | **1.000** | 0.667 | CK T0 表現最佳；Path1 出現 FP+FN（MILESTONE 混淆） |
| gpt-5-mini (none) | 0.667 | 0.857 | CK T0 輸出了 10 個標籤（預期 5 個），產生 5 個 FP |
| gpt-5-mini (full) | 0.667 | 0.667 | 同樣的 FP 問題；Path1 推理並未帶來品質增益 |

**核心發現：**

1. **gpt-5-mini 完全推理版不適合正式環境**：TTFT 平均長達 42 秒。在串流 SSE 情境下，使用者約 40 秒看不到任何回應，這會破壞使用者體驗。故排除。
2. **`reasoning_effort: "none"` 解決了 TTFT 問題**（降至 3.8 秒），但由於回應字數過長（平均 2,300 字，對比 gpt-4o-mini 僅 485 字），總體延遲仍維持在約 24 秒。過多的贅字並非代表品質，而是在相同的標籤區塊周圍產生了不必要的贅述。
3. **gpt-4.1-mini 在 CK T0 展現了最高的天花板 (F1 1.000)** 且延遲在合理範圍內，但在 Path1 T0 出現衰退。在正式採用前需進行完整的 13 輪 F1 評測。
4. **gpt-4o-mini 依然最為穩定**，跨案例表現平均且總體速度最快。

**決策：** 針對 **gpt-4.1-mini** 執行完整的 13 輪 State Tag F1 評測，將其作為下一代正式環境模型的首選。gpt-5-mini (`reasoning_effort: "none"`) 則作為備選方案，前提是需解決首輪標籤過量發送 (FP) 的問題。

### Run 9 — gpt-4.1-mini 評測 (2026-04-18)

**變更：** 將回答 LLM 從 `gpt-4o-mini` 切換為 `gpt-4.1-mini`。Prompt 維持 Run 8 版本，無變動。

**結果：**

| 指標 | Run 8 原生 (gpt-4o-mini) | Run 8 正式環境 | Run 9 原生 (gpt-4.1-mini) | Run 9 正式環境 |
| :--- | :---: | :---: | :---: | :---: |
| **Macro F1 (放寬)** | 0.887 | **0.932** | 0.825 | 0.825 |
| **Macro F1 (嚴格)** | 0.849 | **0.894** | 0.810 | 0.810 |
| Micro F1 | 0.873 | **0.914** | 0.835 | 0.835 |
| TP / FP / FN | 31 / 5 / 4 | 32 / 3 / 3 | 33 / 9 / 4 | 33 / 9 / 4 |
| Forbidden 違規次數 | 2 | 1 | **0** | **0** |

**各回合明細：**

| 會話題型 | Run 8 正式 F1-R | Run 9 原生 F1-R | 差異 | 備註說明 |
| :--- | :---: | :---: | :---: | :--- |
| ck_progressive T0–T2 | 1.000 | **1.000** | = | 三回合表現完美 |
| ck_no_assumption T0 | — | 0.571 | — | FP: 多出 2 個標籤；P=0.400, R=1.000 |
| ck_no_assumption T1 | 0.500 | 0.800 | **+0.300** | 部分改善；仍遺漏 1 個 FN |
| bc_salary_tiers T0 | — | 0.857 | — | FP: 多出 1 個標籤 |
| bc_salary_tiers T1 | — | 1.000 | — | 正確 |
| sv_complete T0 | 0.727 ⚠違規 | 1.000 (放寬) / 0.800 (嚴格) | ↑ 放寬進步 | 放寬下完全答對；嚴格下有 1 個 VALUE 格式不符 |
| sv_complete T1 | — | 1.000 | — | 正確 |
| **feg_path_b T0** | **≥ 1.000** | **0.000** | **−1.000** | **嚴重衰退：未發送任何標籤** |
| **feg_path_b T1** | **≥ 1.000** | **0.500** | **−0.500** | 部分恢復；出現 FP+FN |
| ck_path1_direct T0–T1 | 1.000 | **1.000** | = | 維持穩定 |

**關鍵洞察：**

1. **嚴重衰退 — `feg_path_b`**：模型在 T0 完全沒有發送標籤 (F1 0.000)，在 T1 也僅發送部分 (F1 0.500)。gpt-4.1-mini 似乎無法識別《專業人才移民法》(FEG) 簽證類型的相關標籤語法。單是這個對話就將整體的宏觀 F1 從預期約 0.940 拉低到 0.825。
2. **過濾器在 Run 9 毫無作用**：正式環境的 F1 等於原生 F1 — 由於模型天生就能處理 Path 1 (ck_path1_direct T0/T1 原生皆為 1.000)，因此程式端的過濾器並未發揮額外價值。
3. **較高的 FP 數量 (9 vs 5)**：首輪對話持續發生過量發送標籤的情況。這與基準測試的發現一致，反映出模型有在使用者尚未確認事實前，就傾向發送推測性 REQ 標籤的慣性。
4. **零違規記錄**：沒有違反任何禁止標籤發送的規則。

**深度分析：**

**A. gpt-4.1-mini 的真實潛力被單一 Schema 失敗所掩蓋。**
檢視原生 TP 數量：gpt-4.1-mini 產出了 33 個 TP，而 gpt-4o-mini 為 32 個 — 這代表從絕對數量來看，該模型召回 (Recall) 的正確標籤反而更多。FN 的數量與後者持平 (4)。整體的宏觀 F1 落差完全是由首輪 FP 過量發送 (9 vs 5) 加上 `feg_path_b` 歸零導致的。若排除 `feg_path_b`，gpt-4.1-mini 的 F1 很可能會超越 gpt-4o-mini 的原生基準。這是一次 Prompt 指令覆蓋不全導致的失敗，而非模型能力衰退。

**B. 零違規表現揭示了不同的優勢特徵。**
gpt-4.1-mini 達成零違規（相較於 gpt-4o-mini 的 2 次）。這顯示該模型更擅長遵守「負面守則」（不准發送 X），但在「正面 Schema 召回」（發送對應簽證類型 Y）方面較弱。這與針對 FEG 的 Prompt 引導不足相吻合，而非通用的指令遵循能力下降。

**C. `feg_path_b` 的崩潰主要是 Prompt 引導空缺。**
在目前的 `DOMAIN_KNOWLEDGE` 區塊中，對《專業人才移民法》(FEG) 的引導完整度明顯低於機遇卡或藍卡。gpt-4.1-mini 在 T0 完全沒有發送標籤，暗示模型根本沒有意識到該簽證類型需要搭配 REQ 標籤架構，而非無法遵循標籤格式（格式在其他回合表現完全正確）。

**結論：**

gpt-4.1-mini (正式環境 F1 **0.825**) 表現低於 Run 8 基準線 (0.932)。直接原因在於 `feg_path_b` 接軌失敗，而非通用的推理效能倒退。**目前依然以「gpt-4o-mini 搭配後處理過濾器」作為正式環境的基礎配置。** 暫不採用 gpt-4.1-mini；下一步計畫是針對 FEG Path B 增加正面案例引導進行 Prompt 修正，並執行 Run 10 評測。

### ADR 筆記：正式環境過濾器與模型的耦合問題 (Model Coupling) (2026-04-19)

Run 8 與 Run 9 的對比揭示了現行架構中的一個結構性風險：

> **`src/rag/tag_filter.py` 中的後處理過濾器是針對 gpt-4o-mini 的特定失敗模式進行精確調校的。** 當底層模型切換時，過濾器的有效性可能歸零 — 甚至過濾器可能會掩蓋掉那些需要全新規則才能解決的新型失敗模式。

| 模型 | 過濾器增益 (正式 − 原生 F1) | 過濾器狀態 |
| :--- | :---: | :--- |
| gpt-4o-mini (Run 8) | **+0.045** | 有效 — 修復了兩個已知的失敗模式 |
| gpt-4.1-mini (Run 9) | **+0.000** | 閒置 — 模型未觸發過濾器生效條件 |

**架構啟示：**

1. **更換模型並非只需變更一個 `.env` 變數。** 轉換模型的同時，必須重新評估現有的過濾器是否依然適用，以及新的模型失敗模式是否需要全新的過濾規則。
2. **後處理帶來的 F1 增益不具備自動遷移性。** 不能假設 0.932 的正式環境基準可以在不經過完整評測的情況下直接套用到新模型。
3. **過濾器邏輯在程式層級對模型特定行為進行了硬編碼。** 未來所有的 Prompt 優化都必須同時測試原生與正式環境 F1，以區分真正的 Prompt 品質提升與過濾器的補償作用。

**建議實務：** 在評測新模型時，務必執行完整的 13 輪評估並檢視 `prod_f1 > raw_f1` 是否成立。像 Run 9 這種增益持平的訊號，是在提醒我們要去審視哪些舊有的過濾器已成為廢棄代碼 (dead code)，以及是否需要針對新模型開發新的過濾邏輯。

### Run 10 — gpt-4.1-mini + FEG Path B Prompt 修復 (2026-04-19)

**變更：** 在 `src/rag/prompt_builder.py` 中針對 FEG 認可合作夥伴路徑 (Path B) 加入了定向引導。遵循單一變數原則，未修改其他區段。新增內容包含三項：

| 新增項目 | 位置 | 目的 |
| :--- | :--- | :--- |
| FEG REQ 標籤映射表 | Path B 兩步驟說明後 | 提供明確的 條件→標籤 查找，與學生簽證結構一致 |
| 錯誤 / 正確 範例 6 | 範例區塊 | 護理師 + 簽署雇主承諾信 → 必須輸出 `[MILESTONE:1:current] [REQ:1:TBC:warning] [REQ:4:TBC:warning]` |
| MILESTONE:1 觸發清單加入 FEG | MILESTONE 區段 + 自核第 4 點 | 明確列出 `FEG skilled_worker`；強化自核機制 |

**結果：**

| 指標 | Run 8 正式 (gpt-4o + 過濾器) | Run 9 原生 (gpt-4.1-mini) | **Run 10 原生 (gpt-4.1-mini + 修正)** |
| :--- | :---: | :---: | :---: |
| **Macro F1 (放寬)** | 0.932 | 0.825 | **0.952** |
| **Macro F1 (嚴格)** | 0.894 | 0.810 | **0.936** |
| Micro F1 | 0.914 | 0.835 | **0.911** |
| TP / FP / FN | 32 / 3 / 3 | 33 / 9 / 4 | **36 / 6 / 1** |
| Forbidden 違規次數 | 1 | 0 | **0** |
| 過濾器增益 | +0.045 | +0.000 | **+0.000** |

**各回合明細：**

| 會話題型 | Run 9 F1-R | Run 10 F1-R | 差異 | 備註說明 |
| :--- | :---: | :---: | :---: | :--- |
| ck_progressive T0–T2 | 1.000 | 1.000 | = | 維持穩定 |
| ck_no_assumption T0 | 0.571 | 0.571 | = | FP: 2 個標籤過量；維持原狀 |
| ck_no_assumption T1 | 0.800 | 0.800 | = | FN: 遺漏 1 個標籤；維持原狀 |
| bc_salary_tiers T0 | 0.857 | **1.000** | **+0.143** | 意外的改善（並未修改藍卡部分） |
| bc_salary_tiers T1 | 1.000 | 1.000 | = | 維持穩定 |
| sv_complete T0 | 1.000 (放寬) | 1.000 (放寬) | = | 嚴格分差距 (0.800) 並未改變 |
| sv_complete T1 | 1.000 | 1.000 | = | 維持穩定 |
| **feg_path_b T0** | **0.000** | **1.000** | **+1.000** | 全面恢復 — FEG 修復獲得證實 |
| **feg_path_b T1** | **0.500** | **1.000** | **+0.500** | 全面恢復 |
| ck_path1_direct T0–T1 | 1.000 | 1.000 | = | 維持穩定 |
...
1. **Run 10 (原生 0.952) 在完全不依賴後處理過濾器的情況下，超越了 Run 8 的正式環境表現 (0.932)。** 模型層級的品質提升勝過了 Run 8 對 gpt-4o-mini 使用的過濾補償。
2. **針對 FEG 的診斷正確。** 透過新增結構化的 REQ 映射表與單一組「正確/錯誤範例」，`feg_path_b` 的表現從 0.000/0.500 完整恢復至 1.000/1.000。這再次確認了之前的失敗是 Prompt 指引覆蓋不足，而非模型推理能力問題。
3. **`bc_salary_tiers` T0 出現額外增益 (0.857 → 1.000)**，儘管完全未更動藍卡規則。這顯示 Prompt 中更明確的正面範例結構，有助於提升模型在首輪標籤發送上的整體紀律。
4. **剩餘缺失**（`ck_no_assumption` T0: 0.571, T1: 0.800, `sv_complete` T0 嚴格: 0.800）與 Run 9 相比維持不變，再次證實 FEG 修復確實是單一變數改進。

**結論：**

**正式採用「gpt-4.1-mini 搭配更新後的 Prompt」作為新的正式環境基準。宏觀 F1 分數 0.952 成為新的技術標竿。** 原有的 `tag_filter.py` 過濾器依然保留作為安全網，但目前對此模型來說純屬閒置 (增益為 0)。Run 11 目標：解決 `ck_no_assumption` T0 首輪對話標籤過量發送 (FP=2, P=0.400) 的問題。

---

### Run 10 泛化能力驗證 — FEG Path B 換詞測試 (2026-04-19)

**動機：** Prompt 中的「範例 6」直接描述了與 `feg_path_b` 測試案例相似的情境（台灣護理師、學位未獲認可、已簽雇主承諾）。Run 10 從 0.000 躍升至 1.000 的表現引發了「測試集外洩」的疑慮：模型是真的學會了規則，還是只是在比對範例的表面形式？

**方法：** 在 `eval/feg_paraphrase_dataset.json` 中準備了 5 個未登錄的換詞變體。每個變體都描述相同的 Path B 觸發場景，但使用了不同的職業、國籍、措辭與輸入語言。所有的表面特徵（國籍、職業、醫院名）皆與 Prompt 中的範例不重疊。

| ID | 描述 | F1 |
| :--- | :--- | :---: |
| `feg_paraphrase_1` | 韓國物理治療師、法蘭克福復健診所 | **1.000** |
| `feg_paraphrase_2` | 抽象的法條 §16d 引用 — 未指定職業或國籍 | **1.000** |
| `feg_paraphrase_3` | 德文輸入 ("Anerkennungspartnerschaft unterzeichnet") | **1.000** |
| `feg_paraphrase_4` | 印度機械工程師、柏林製造商 | **1.000** |
| `feg_paraphrase_5` | 越南護理師、口語化的模糊措辭 ("支持流程"，未使用專業術語) | **1.000** |

**加總：** 宏觀 F1 = **1.000** (15 TP / 0 FP / 0 FN)，0 次違規。

**判定：證實具備真正的泛化能力。** 模型在 5 個換詞變體中均正確輸出了 `[MILESTONE:1:current] [REQ:1:TBC:warning] [REQ:4:TBC:warning]`。亮點包含：

* `feg_paraphrase_2`：即使沒有職業或國籍，僅憑抽象的 §16d Schema 引用仍能正確觸發。
* `feg_paraphrase_5`：未使用任何專業術語（無 "Anerkennungspartnerschaft" 或 "§16d"），僅提到「雇主願意等待並支持申請」，依然能正確觸發。
* `feg_paraphrase_3`：使用德文輸入，證實了跨語言的穩健性。

**啟示：** Run 10 新增的結構化 REQ 映射表確實讓模型學會了底層的規則結構（雇主承諾 → 等待認可 → A2 持疑），而非僅是表面模式的比對。Run 10 的品質改善是真實且具備深度的。

---

### Run 11 — gpt-4.1-mini + 機遇卡計分標籤壓制 (2026-04-19)

**變更（單一變數）：** 將「首輪標籤發送規則」拆分為兩條子規則：
* **規則 A (門檻標籤 REQ:1-x)**：無論使用者是否提及，皆預設初始化為 TBC:warning。
* **規則 B (計分標籤 REQ:2-x)**：僅在使用者明確提供可計分事實時才發送；絕不隨便初始化為 TBC|0。

配合新增「範例 7」：展示「一般性諮詢，未提供個人細節 → 僅初始化進入門檻標籤」。自核項目第 4 點同步更新。

**動機：** Run 10 的 `ck_no_assumption T0` 輸出了 10 個標籤 (F1=0.571)，包含 4 個預期的門檻標籤與 6 個多餘的計分標籤 (`REQ:2-x:TBC|0:warning`)。這顯示模型過於死板地遵循了「初始化完整核檢清單」的指令。

**結果：**

| 指標 | Run 10 原生 | **Run 11 原生** | Run 10 正式環境 | **Run 11 正式環境** |
| :--- | :---: | :---: | :---: | :---: |
| **Macro F1 (放寬)** | 0.952 | **0.903** | 0.952 | **0.974** |
| **Macro F1 (嚴格)** | 0.936 | **0.903** | 0.936 | **0.974** |
| Micro F1 | 0.911 | **0.917** | 0.911 | **0.972** |
| TP / FP / FN | 36 / 6 / 1 | **33 / 3 / 3** | 36 / 6 / 1 | **35 / 1 / 1** |
| Forbidden 違規次數 | 0 | **1** | 0 | **0** |

**各回合明細：**

| 會話題型 | Run 10 F1-R | Run 11 F1-R | Run 11 正式 F1-R | 差異 | 備註說明 |
| :--- | :---: | :---: | :---: | :---: | :--- |
| ck_progressive T0–T2 | 1.000 | 1.000 | 1.000 | = | 維持穩定 |
| **ck_no_assumption T0** | **0.571** | **1.000** | **1.000** | **+0.429** | ✅ 目標達成 — 壓制了 REQ:2-x |
| **ck_no_assumption T1** | **0.800** | **1.000** | **1.000** | **+0.200** | ✅ 額外的改善 |
| bc_salary_tiers T0 | 1.000 | 1.000 | 1.000 | = | 維持穩定 |
| bc_salary_tiers T1 | 1.000 | 0.667 | 1.000 | −0.333 原生 / = 正式 | MILESTONE:2 前進失敗（已被過濾器修復） |
| sv_complete T0 | 1.000 | 1.000 | 1.000 | = | 維持穩定 |
| sv_complete T1 | 1.000 | 0.667 | 0.667 | −0.333 | 既有的 MILESTONE:2 + PRESERVE 缺口（見下文） |
| feg_path_b T0–T1 | 1.000 | 1.000 | 1.000 | = | 維持穩定 |
| ck_path1_direct T0 | 1.000 | 1.000 | 1.000 | = | 維持穩定 |
| ck_path1_direct T1 | 1.000 | 0.400 | 1.000 | −0.600 原生 / = 正式 | MILESTONE:2 前進 + Path 1 語言標籤誤報（已被過濾器修復） |

**關鍵洞察：**

1. **正式環境 F1 = 0.974 — 創下技術新高**，超越了 Run 10 原生與正式環境的 0.952 分。原生分數的下降 (0.952 → 0.903) 反映出 MILESTONE:2 階段推進失敗的問題再次浮現，這屬於模型在隨機性波動下的變異，而非此次變動導入的衰退。
2. **`ck_no_assumption T0` 問題獲得完美解決。** 該回合的 FP 從 6 個降至 0。模型不再於一般性諮詢中初始化計分相關的所有 6 個子標籤。
3. **`ck_no_assumption T1` 也隨之進步 (0.800 → 1.000)** — 這可能是受益於更乾淨的首輪狀態（沒有多餘的計分 TBC 標籤需要繼承）。
4. **`sv_complete T1` 的原生衰退 (1.000 → 0.667) 是既有的且尚未修復的問題**：模型重發了 `MILESTONE:1:current` 而未邁向 `MILESTONE:2`，同時重發了 `REQ:2:TBC:warning` 違反了 PRESERVE 規則。這些都與 Run 11 針對首輪計分標籤的更動無關 — 這是模型在 temperature=0.1 下的邊際波動。Run 12 目標：強化 MILESTONE:2 前進的穩健性。
5. **過濾器增益變得極具意義**（Run 11 宏觀 F1 提升了 +0.071），這與 Run 10 增益為 0 的情況不同。過濾器成功剔除了重發的 MILESTONE:1 以及 T1 回合中的 PRESERVE 違規標籤。

**結論：**

正式採用「Run 11 正式環境 F1 = 0.974」作為新的基準配置。這些安插的邏輯查驗器有辦法對付那些 Prompt 指令始終勸說不聽的死角罩門，至於那些跟漏給標籤牽扯的特定死角缺陷 (這是不可能憑藉過濾器代打的 False Negatives 漏報情形) 則被認列為可接受的系統極限。目前的評鑑框架就此將原始直出 F1 (評比 Prompt 功力極限) 與正式環境 F1 (用戶真實有感準確度) 分開測量追蹤，作為爾後再行標竿。
