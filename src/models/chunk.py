...
class ChunkMetadata(BaseModel):
    ...
    authority_level: AuthorityLevel = Field(
        default=AuthorityLevel.THIRD_PARTY, description="Credibility level of source"
    )
    visa_types: List[VisaType] = Field(default_factory=list, description="Relevant visa categories")
    ...
    parent_is_main_heading: bool = Field(default=False, description="Parent chunk represents main H2 section")
    ...
    # Link tracking
    referenced_urls: List[str] = Field(default_factory=list, description="URLs referenced in chunk text")
...
class Chunk(BaseModel):
    ...
    dense_vector: Optional[List[float]] = Field(
        default=None, description="Dense embedding vector (1536-dim for text-embedding-3-small)"
    )
    sparse_vector: Optional[Dict[int, float]] = Field(default=None, description="Sparse BM25 vector for hybrid search")
