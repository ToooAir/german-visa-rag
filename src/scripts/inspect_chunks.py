...
def inspect_chunks(sample_size: int = 5, collection_name: str = None):
    ...
    try:
        ...
        for i, point in enumerate(results):
            ...
            issues = []
            if len(text) < 100:
                issues.append("⚠️ Too short (<100 chars)")
            if "cookie" in text.lower() or "privacy policy" in text.lower():
                issues.append("⚠️ Potential web noise (cookies/legal)")
            if not header or header == "Introduction":
                issues.append("ℹ️ generic header context")
